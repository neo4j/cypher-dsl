...
public final class Follows_ extends RelationshipImpl<Person_, Person_, Follows_> {
	public Follows_(Person_ start, Person_ end) {
		super(start, "follows", end);
	}

	private Follows_(SymbolicName symbolicName, Node start, String type, Properties properties,
			Node end) {
	...
	public Follows_ named(SymbolicName newSymbolicName) {
		return new Follows_(newSymbolicName, getLeft(), getRequiredType(), getDetails().getProperties(), getRight());
	}

	@Override
	public Follows_ withProperties(MapExpression newProperties) {
		return new Follows_(getSymbolicName().orElse(null), getLeft(), getRequiredType(), Properties.create(newProperties), getRight());
	}
}