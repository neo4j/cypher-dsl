	...
	void includeAllShouldWork() {
		...
		try (var session = driver.session()) {
			try {
				...
				var nodes = session.run(query)
					.stream().map(r -> r.get("testNode").asNode().get("name").asString());
				Assertions.assertThat(nodes).containsExactly("n1");
			} finally {
				session.run("MATCH (n:IncludeAllNode) DELETE n").consume();
			}
		}
	}
	...
	void includeAnyShouldWork() {
		...
		try (var session = driver.session()) {
			try {
				...
				session.run("MATCH (n:IncludeAnyNode) DELETE n").consume();
			}
		}
	}
}
